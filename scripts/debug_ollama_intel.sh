...
####################################
#
# Ollama Intel Iris Xe GPU Debug Script
...
echo -e "${BLUE}=========================================${NC}"
echo -e "${BLUE}Ollama Intel Iris Xe GPU Debug Script${NC}"
...
# 8. Ollama Status
echo -e "${YELLOW}[8] Ollama Status${NC}"
if command_exists ollama; then
    echo -e "${GREEN}✓ Ollama installed${NC}"
    echo "Version: $(ollama version 2>/dev/null || echo 'Unable to get version')"
    ...
    env | grep -i ollama || echo "No Ollama environment variables set"
...
echo -e "${YELLOW}3. Set Ollama environment for Intel GPU:${NC}"
echo "   export OLLAMA_GPU_OVERRIDE=1"
echo "   export ZE_AFFINITY_MASK=0"
echo ""

echo -e "${YELLOW}4. Test Ollama with a small model:${NC}"
echo "   ollama run phi3:mini"
echo ""

echo -e "${YELLOW}5. Monitor GPU usage:${NC}"
echo "   intel_gpu_top  # Install with: sudo apt install intel-gpu-tools"
echo ""
